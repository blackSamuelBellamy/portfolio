import { motion } from "framer-motion";
import { styles } from "../styles";
import { SectionWrapper } from "../hoc";
import { fadeIn, textVariant } from "../utils/motion";
import { testimonials } from "../constants";

const FeedbackCard = (props) => {
  return (
    <motion.div
      variants={fadeIn("", "spring", props.index * 0.5, 0.75)}
      className="bg-black-200 p-10 rounded-3xl xs:w-[320px] w-full"
    >
      <p className="text-white font-black text-[48px]">"</p>
      <div className="mt-1">
        <p className="text-white tracking-wider text-[18px}">{props.testimonial}</p>
        <div className="mt-7 flex justify-between items-center gap-1">
          <div className="flex flex-1 flex-col">
            <p className="text-white font-medium text-[16px]">
              <span className="blue-text-gradient">@</span> {props.name}
            </p>
            <p className="text-secondary mt-1 text-[12px]">
              {props.designation} of {props.company}
            </p>
          </div>
          <img 
            src={props.image}
            alt={`feedback-by-${props.name}`}
            className='w-11 h-11 rounded-full object-cover'
          />
        </div>
      </div>
    </motion.div>
  );
};

const Feedbacks = () => {
  return (
    <div className="mt-12 bg-black-100 rounded-[20px]">
      <div
        className={`${styles.padding} bg-tertiary rounded-2xl min-h-[300px]`}
      >
        <motion.div variants={textVariant()}>
          <p className={styles.heroSubText}>Acerca de mí</p>
          <h2 className={styles.heroHeadText}>Testimonio de docentes</h2>
        </motion.div>
      </div>
      <div className={`${styles.paddingX} mt-[-30px] pb-14 flex flex-wrap gap-7`}>
        {testimonials.map((x, i) => (
          <FeedbackCard key={`testimonio-${i}`} index={i} {...x} />
        ))}
      </div>
    </div>
  );
};

export default SectionWrapper(Feedbacks);
